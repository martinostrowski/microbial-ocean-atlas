...
dir.create(paste0(export_dir, "pseudo.run_255_250"));
trimmed_dir <-(paste0(plates[p,1], ".trimmed/"));
dir.create(paste0(trimmed_dir));
trunc_dir <-(paste0(export_dir, "pseudo.run_255_250"));
...
list.files(path=base.dir,  pattern=".fastq");
data.fp.gz <- paste0(plates[p,1], "/");
print(data.fp.gz);
...
FWD <- "AGAGTTTGATCMTGGCTCAG";  ## CHANGE ME Bacteria 16S: 27f V1_V3
...
out <- filterAndTrim(fnFs.cut, trimFs, fnRs.cut, trimRs, truncLen=c(255,250),
