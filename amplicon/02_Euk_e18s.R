...
dir.create(paste0(export_dir, "pseudo.run_200_195"));
trimmed_dir <-(paste0(plates[p,1], ".trimmed/"));
dir.create(paste0(trimmed_dir));
trunc_dir <-(paste0(export_dir, "pseudo.run_200_195"));
trimLeng <-(paste0(trimmed_dir, "pseudo.run_200_195"));
...
FWD <- "CCAGCASCYGCGGTAATTCC";  ## CHANGE ME EUK 18S forward primer
REV <- "ACTTTCGTTCTTGATYRATGA"; ## CHANGE ME...EUK 18S reverse
...
#STEP7:  DEREPLICATION, DADA2 Step, MERGE, and SAVE RDS 
#(***when running multiple plates you will run a collapse no mismatch step at the end on all plates together, if running one plate, you can run collapse here)
...
track.table4$truncL <- 200;
track.table4$truncR <- 195;
track.table4$EE <- 2.6;
...
track.table1$truncL <- 200;
track.table1$truncR <- 195;
track.table1$EE <- 2.6;
